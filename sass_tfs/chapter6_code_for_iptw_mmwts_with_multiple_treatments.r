...
#===================================================
#this code runs baseline category logit model to estimate generalized propensity scores
require(VGAM)
ps.model <- vglm(psFormula,weights=imputedData$TFNLWGT, family=multinomial, data=imputedData)
ps <- data.frame(fitted(ps.model))


#obtain minimums and maximums to check common support
...
densityplot( ~ps|time, groups=Treat, plot.points=F, auto.key=T, 
             data = reshape(data.frame(ps,imputedData[,c("CNTLNUM","Treat")]),
                            idvar="CNTLNUM",varying=c("noMentor","sameArea","otherArea"), v.names="ps",
                            times=c("GPS of No Mentor","GPS of Same Area","GPS of Other Area"),direction="long"), 
...
require(twang)
set.seed(2016) #set random seed so that results are replicable
boost.ps <- mnps(psFormula, data = imputedData,
  estimand = "ATE", verbose = FALSE,
  stop.method = c("es.max"), #stop criterion is maximum effect size
  n.trees = 10000, #number of trees
  sampw = imputedData$TFNLWGT) #sampling weights

summary(boost.ps)



#save(list=ls(), file="chapter6_results.Rdata", compress=T)

#visual inspection of convergence
tiff("Chapter6_figure6-2.tif", res=600, compression = "lzw", height=6, width=15, units="in")
...
#request box-and-whiskers plots to evaluate common support
tiff("Chapter6_boxplot.tif", res=600, compression = "lzw", height=18, width=15, units="in")
plot(boost.ps, color=F, plots = 2, figureRows=1)
dev.off()

#request density plot to evaluate common support
require(lattice)
lattice.options(default.theme = standard.theme(color = FALSE))
tiff("Chapter6_figure6-3.tif", res=600, compression = "lzw", height=6, width=15, units="in")
densityplot( ~ps|time, groups=Treat, plot.points=F, auto.key=T, 
             data = reshape(data.frame(ps2,imputedData[,c("CNTLNUM","Treat")]),
                            idvar="CNTLNUM",varying=c("noMentor","sameArea","otherArea"), v.names="ps",
                            times=c("GPS of No Mentor","GPS of Same Area","GPS of Other Area"),direction="long"), 
             ylab = "Generalized Propensity Scores", xlab = "Treatment Conditions")
dev.off()
...
imputedData$finalWeightATE <- with(imputedData,IPTW.TFNLWGT/
                                    mean(IPTW.TFNLWGT))
...
imputedData$finalWeightATE2 <- get.weights(boost.ps, stop.method = "es.max",   estimand = "ATE",
                                 withSampW = TRUE)
#normalize weights
imputedData$finalWeightATE2 <- with(imputedData,finalWeightATE2 /
                                    mean(finalWeightATE2 ))
#check distribution of weights
...
data$Treat <- as.numeric(data$Treat==condition1)
  balance.iptw <- bal.stat(data=data,#dataset is defined
  ...
  return( balance.iptw$results) } #close function
...
#identify covariates/categories with standardized mean difference above 0.1, which will be 
#used in covariance adjustment
...
#=============================================
#evaluate covariate balance with the twang function using the 
#generalized propensity scores obtained with generalized boosted regression

#visual inspection of covariate balance
require(twang)
tiff("Chapter6_figure6-4.tif", res=600, compression = "lzw", height=6, width=15, units="in")
...
write.csv(summary.balance.boost, file="summary_balance_with_generalized_boosted_regression.csv")

#identify covariates with standardized mean difference above 0.1, which will be 
#used in covariance adjustment
...
mmws.weights <- function(id,treat, ps,numb.strata) {
  ...
  #looop through levels of treatment
  ...
  } #close loop through levels of treatment
  
  #create a single weight
  all.weights$mmws <- apply(all.weights[,-1],1,sum)

  return(all.weights[,c("id","mmws")]) } #close function
#=========================================================  

#run function to obtain MMWS
...
#including covariance adjustment for covariates that did not achieve
...
#save all results
save(list=ls(), file="Chapter_6_all_results.Rdata", compress=T)
