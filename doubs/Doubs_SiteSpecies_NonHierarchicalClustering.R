...
packages <- c("dplyr", "vegan", "cluster")
...
# ------------------------------------------
# Silhouette plot of the final partition and Compare with Ward hierarchical clustering
...
source("./RefData/NumericalEcologyWithR/NEwR2-Functions/drawmap.R")
par(mfrow = c(1, 2))
...
# ------------------------------------------------------------------------------
# Normality test of residuals --> normally distributed
...
# Homogeneity of variances --> variable sqrt(ele) has heterogeneous variances, not appropriate for parametric ANOVA
...
# Look at the corrected permutational p-values. If all values are equal to or smaller than 0.05, you can consider that all groups are globally significant, i.e. that on the whole
# they contain species that are concordant. (only at least some species are)
...
# If a species has anegative mean correlation with all other members of its group, this indicates that this species should be left out of the group
...
# Now all species in the 3 groups have positive mean Spearman correlations with the other memvers of their group.
...
#    and assessing its probability by means of a permutationtest in the spirit of the Raup and Crick coefficient.
...
adjm4 <- picante::species.dist(spe.pa, metric = "jaccard") # Jaccard!!
...
(iva2 <- multipatt(spe, grps, max.order = 2, control = how(nperm = 999)))
...
# ------------------------------------------------------------------------------
# Correlation-type Index for presence-absence data:  Pearson's phi coefficient of association
# ------------------------------------------------------------------------------
...
# compute bootstrap confidence intervals around the phi values.
