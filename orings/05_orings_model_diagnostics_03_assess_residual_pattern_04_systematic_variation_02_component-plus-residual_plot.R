...
car::crPlots(mod_obj, ~ temp, smooth = list(span = 0.5), id = TRUE)

# here no covariates other than temp
car::ceresPlots(mod_obj, ~ temp, smooth = list(span = 0.5), id = TRUE)
...
car::crPlots(mod_obj, ~ poly(temp, 2), smooth = list(span = 0.5), id = TRUE)
