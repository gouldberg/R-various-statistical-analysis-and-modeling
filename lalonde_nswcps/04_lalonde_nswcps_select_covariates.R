...
data <- as.data.frame(cps1_nsw_data)
...
balanceTable_0$coef_to_treat <- balanceTable_0$coef_to_treat / sd(data$treat)
...
for (var in 1:length(covariateNames0)) {
  ...
  formula02 <- paste("re78 ~ ", covariateNames0[var], sep = "")
...
balanceTable_02$coef_to_outcome <- balanceTable_02$coef_to_outcome / sd(data$re78)
