...
packages <- c("dplyr", "vegan", "ape", "spdep", "ade4", "adegraphics", "adespatial")
...
# ------------------------------------------------------------------------------
# Univariate spatial correlogram (based on Moran's I) of substrate density
# ------------------------------------------------------------------------------
...
source("./RefData/NumericalEcologyWithR/NEwR2-Functions/plot.links.R")
...
# This correlogram has a single singnificant distance class:  there is positive spatial correlation at distance class 1 (i.e., 0.0 m to 0.7 m)
# Negative spatial correlation at distance class 4 (2.1 m to 2.8 m) is hinted at, but the coefficient is not significant after Holm correction for multiple testing.
plot(subs.correlog)


# --> measurements taken more than 0.7 m or (conservatively) 2.8 m apart (the upper limit of class 4) can be considered as spatially independent with respect to substrate density.
...
#  - This condition is that the spatial variation of the dat should be adequately described by the same single spatial correlation function in all portions of the study area.
...
# Number of classes is computed using Sturge's rule = 1 + (3.3219 * log10(n)),
# where n is number of elements, here the number of pairwise distances
# or: mite.correlog[2]
...
# Any pair of sites whose members are about 2 - 4 m apart falls into constrasting soil conditions, which in turn explains why their mite communities are different.
...
# Computation of a raw (non-orthogonal) third-degree polynomial function on the previously centred X-Y coordinates
