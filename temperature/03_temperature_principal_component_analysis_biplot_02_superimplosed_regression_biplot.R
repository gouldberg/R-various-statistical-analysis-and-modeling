...
round(t(pca_temp2$rotation[,1:2]), 3)
...
# ----------
# Orthogonal projections on January  (R^2 is largst 0.994)
# R^2 values for response 400m is high and we can trust the projections on January axis. 
...
calibrate::calibrate(fitlms$coef[,"January"], Y[,"January"], tm = seq(-2, 2, by = 0.5), Fr = X, dp = TRUE, 
                   axiscol = "brown", axislab = "January", labpos = 3, verb = FALSE)




# ----------
# Orthogonal projections on May axis  (R^2 is smallest 0.962)
# plot(X[,1], X[,2], pch = 20, xlab = "PC1", ylab = "PC2", col = "darkblue", asp = 1, main = "Biplot Axis",  xlim = c(-3.2, 3.2))
# text(X[,1], X[,2], labels = rownames(X), cex = 0.7, pos = 3, col = "darkblue")
# abline(h = 0, v = 0, lty = 2, col = "gray")
