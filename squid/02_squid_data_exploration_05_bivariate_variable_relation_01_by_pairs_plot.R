...
formula <- ~ Testisweight + Specimen + DML + fMONTH


scatterplotMatrix(formula, data = Squid,
                  smooth = FALSE,
...
# ----------
# data transformation
# formula <- ~ Testisweight^(0.33) + Specimen^(-0.396) + DML + fMONTH
Squid_tmp <- Squid %>% mutate(Testisweight2 = Testisweight^(0.33), Specimen2 = Specimen^(-0.396))
formula <- ~ Testisweight2 + Specimen2 + log(DML) + fMONTH

scatterplotMatrix(formula, data = Squid_tmp,
...
scatterplotMatrix(formula, data = Squid, groups = Squid$Testisweight >= 20,
                  smooth = FALSE, ellipse = list(levels = 0.5), 
...
var_obj <- colnames(Squid)


# here we apply method = "spearman" due to terrible skewness
pairs.panels(Squid[, var_obj], ci = TRUE, smoother = TRUE, stars = TRUE, method = "spearman")
...
MyVar <- c("DML", "YEAR", "MONTH", "Specimen")


Myxyplot(Squid, MyVar, "Testisweight")
