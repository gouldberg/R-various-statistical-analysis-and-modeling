...
for(k in 1:40){
  sigma2 <- ar(inf, order = k, aic = FALSE)$var.pred
...
# ------------------------------------------------------------------------------
# Spectral analysis:  Autoregressive spectral estimator for logged Inflow
# ------------------------------------------------------------------------------
...
spaic <- spec.ar(inf, log = "no")

# peak
abline(v = 40 / 480, lty = 3)
...
# ----------
# estimates and AICs
( inf.ar <- ar(inf, order.max = 40) )

inf.ar$aic

plot(1:40, inf.ar$aic[-1], type = "o")
