...
x.new <- llar.predict(flu, n.ahead = 20, m = 3, eps = 0.1, onvoid = "enlarge", r = 5)
...
x.fitted <- llar.fitted(flu, m = m, eps = 0.8)
...
lines(ts(x.fitted, start = 0+m, end = length(flu)+m), col = "blue", lty = 2)
...
spaceTimePlot(time.series = flu, embedding.dim = 3, time.lag = 10,
              time.step = 1, max.radius = 1, numberPercentages = 10, do.plot = TRUE,
...
# ----------
fn <- false.nearest(flu, m.max, d, tw, rt, eps)
...
recurr(flu, m = 2, d = 3, levels = c(0,0.2,0.4,0.6,1))
