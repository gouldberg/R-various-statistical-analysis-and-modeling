...
Tl.eq <- log(output) ~ log(labor)

Tl.eq <- log(output) ~ log(machine)

plotplm(plm(Tl.eq, data = Tl.p), N = 10, seed = 4, lgth = .05)



# -->
# The individual effects are negatively correlated with the covariate,
# and consequently, the within estimator is much higher.
