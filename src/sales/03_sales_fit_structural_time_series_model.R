...
modLocallevel <- SSModel(dat$fabrics ~ SSMtrend(1, Q = NA), H = NA)
...
# ----------
# plot filtered value:  slightly different estimation
graphics.off()
...
modTrend <- SSModel(dat$fabrics ~ SSMtrend(2, Q = c(list(0), list(NA))), H = NA)
...
modLocaltrend <- SSModel(dat$fabrics ~ SSMtrend(2, Q = c(list(NA), list(NA))), H = NA)
...
ts.plot(dat$fabrics, type = "b", ylim = c(650, 1150), ylab = "level")
...
lines(smooth.spline(dat$fabrics, spar = 0.75), lwd = 2, col = "blue", lty = 2)
...
( strtrend <- StructTS(dat$fabrics, type = "trend") )
