...
rsvec <- randomstress(n = nrow(OCP), ndim = 2, nrep = 500, type = "ordinal")
...
# 0.37  --> our MDS solution is smaller than this value
...
# which is clearly the case in our example.



# ------------------------------------------------------------------------------
# Simulate Goodness-of-fit in MDS by permutation test
#   - The random stress criterion is not very sharp, as Cliff (1973) puts it, it tests the "nullest of all null hypotheses".
...
permmds1 <- permtest(fit.ocp1, data = OCP[,1:54], method.dat = "euclidean", nrep = 500, verbose = FALSE)

# IT TAKES TIME !!! (5 min ?)
permmds3 <- permtest(fit.ocp3, data = OCP[,1:54], method.dat = "euclidean", nrep = 500, verbose = FALSE)
...
for(i in 1:(n - 1)){ svec[i] <- mds(ocpD[,1:54], ndim = i, type = "ordinal")$stress }
...
for(i in 1:100){ fit.ocp[[i]] <- mds(ocpD, type = "ordinal", init = "random") }



# ----------
# extract the best solution
ind <- which.min(sapply(fit.ocp, function(x) x$stress))
