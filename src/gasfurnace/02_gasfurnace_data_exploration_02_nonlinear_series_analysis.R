setwd("C:\\Users\\kswad\\OneDrive\\デスクトップ\\技術力強化_統計解析\\51_解析スクリプト\\gasfurnace")
...
x.new <- llar.predict(gasf$output, n.ahead = 20, m = 3, eps = 1, onvoid = "enlarge", r = 5)
...
x.fitted <- llar.fitted(gasf$output, m = m, eps = 1)
...
lines(ts(x.fitted, start = 0+m, end = 296+m), col = "blue", lty = 2)
...
spaceTimePlot(time.series = gasf$output, embedding.dim = 3, time.lag = 10,
...
eps <- sd(gasf$output) / 10



# ----------
fn <- false.nearest(gasf$output, m.max, d, tw, rt, eps)
...
mutual(gasf$output, lag.max = 20)



# -->
# 1st minimum average mutual information at lag 6 (time delay = 6)
...
delta.test(gasf$output, m = 2:6)



# delta test of linearity based on conditional mutual information
delta.lin.test(gasf$output)



# -->
# We did not reject conditional independence.
...
recurr(gasf$output, m = 3, d = 2, levels = c(0,0.2,0.4,0.6,1))

# recurr(series = gasf$input, m = 2, d = 2)


plot(gasf$output, type = "l")
abline(v = c(80, 120, 210), col = "blue", lty = 2)
...
s <- ssa(gasf$output, 50, kind = "toeplitz-ss")
