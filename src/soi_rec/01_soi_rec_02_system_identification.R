...
soi <- read.csv("soi.txt", sep = "", header = T, colClasses = "numeric")

rec <- read.csv("rec.txt", sep = "", header = T, colClasses = "numeric")


# convert to idframe
soirec <- idframe(output = ts(rec), input = ts(soi), Ts = 1)
...
# split data
ze <- dataSlice(soirec, start = 1, end = 250)

zr <- dataSlice(soirec, start = 251, end = 453)
...
# -->
# 20 * log10(mean deviation ratio) = 38.88
...
( parms <- expand.grid(na = 1:10, nb = 1:10, nk = 5) )
...
lsimplot(sys, u = c(ze$input), t = 1:250)
...
( parms <- expand.grid(na = 1:5, nb = 1:5, nc = 1:3, nk = 5) )
...
# ----------
modarmax <- armax(ze, order = c(5,2,3,5))
...
plot(resid_arx, type = "l", main = "ARX train", ylim = ylim)
abline(h = 0, col = "gray")

plot(resid_arx_t, type = "l", main = "ARX test", ylim = ylim)
abline(h = 0, col = "gray")

plot(resid_oe, type = "l", main = "OE train", ylim = ylim)
abline(h = 0, col = "gray")

plot(resid_oe_t, type = "l", main = "OE test", ylim = ylim)
abline(h = 0, col = "gray")

plot(resid_armax, type = "l", main = "ARMAX train", ylim = ylim)
abline(h = 0, col = "gray")

plot(resid_armax_t, type = "l", main = "ARMAX test", ylim = ylim)
abline(h = 0, col = "gray")
