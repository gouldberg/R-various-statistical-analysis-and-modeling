...
# diagnose models
# ------------------------------------------------------------------------------
testResults_ridge <- data.frame(obs = solTestY, ridge = predict(ridgeTune, solTestXtrans))
testResults_ridge <- testResults_ridge %>% mutate(resid = obs - ridge)

testResults_lasso <- data.frame(obs = solTestY, lasso = predict(enetTune, solTestXtrans))
...
# Both methods have similar predictive ability
...
ridgeModel <- elasticnet::enet(x = as.matrix(solTrainXtrans), y = solTrainY, lambda = as.numeric(ridgeTune$bestTune))
...
ridgeCoef <- predict(ridgeModel, newx = as.matrix(solTestXtrans), s = 1, mode = "fraction", type = "coefficients")
