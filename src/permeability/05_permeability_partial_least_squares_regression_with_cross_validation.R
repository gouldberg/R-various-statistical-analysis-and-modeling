...
fp <- data.frame(fingerprints)
pm <- data.frame(pm = permeability[,1])
...
# ------------------------------------------------------------------------------
# Remove nearZeroVar variable
# ------------------------------------------------------------------------------
# 719 out of 1,107 variables are removed
remove_var <- nearZeroVar(fp)
...
# Preprocess data
# ------------------------------------------------------------------------------
trans <- preProcess(data.frame(fp), method = c("BoxCox", "center", "scale", "pca"))
trans2 <- preProcess(data.frame(fp), method = c("BoxCox", "center", "scale"))

trans
trans2

fp_trans <- predict(trans, fp)

fp_trans2 <- predict(trans2, fp)
...
# ------------------------------------------------------------------------------
fp_trans$id <- 1:nrow(fp_trans)
fp_trans2$id <- 1:nrow(fp_trans2)
pm$id <- 1:nrow(pm)
...
prob <- 0.75
train_id <- sample(1:nrow(pm), size = floor(nrow(pm) * prob), replace = FALSE)
test_id <- setdiff(1:nrow(pm), train_id)

trainX <- fp_trans %>% filter(id %in% train_id) %>% dplyr::select(-id)
testX <- fp_trans %>% filter(id %in% test_id) %>% dplyr::select(-id)

trainX2 <- fp_trans2 %>% filter(id %in% train_id) %>% dplyr::select(-id)
testX2 <- fp_trans2 %>% filter(id %in% test_id) %>% dplyr::select(-id)



# ----------
trainY <- pm %>% filter(id %in% train_id) %>% dplyr::select(pm)
testY <- pm %>% filter(id %in% test_id) %>% dplyr::select(pm)



# ----------
trainingData <- trainX
trainingData$pm <- trainY$pm

testData <- testX
testData$fat <- testY$pm

trainingData2 <- trainX2
trainingData2$pm <- trainY$pm

testData2 <- testX2
testData2$pm <- testY$pm
...
indx <- createFolds(as.matrix(fp), returnTrain = TRUE)
...
plsTune <- train(x = trainX2, y = trainY$pm,
                 method = "pls",
...
pcrTune <- train(x = trainX2, y = trainY$pm,
                 method = "pcr",
                 tuneGrid = expand.grid(ncomp = 1:40),
                 trControl = ctrl)
...
# diagnose models
# ------------------------------------------------------------------------------
testResults_pls <- data.frame(obs = testY$pm, PLS = predict(plsTune, testX2))
testResults_pls <- testResults_pls %>% mutate(resid = obs - PLS)

testResults_pcr <- data.frame(obs = testY$pm, PCR = predict(pcrTune, testX2))
...
# -->
# The residuals appear to be randomly scattered about 0 with respect to the predicted values.  --> THIS IS NOT ...
...
# confidence interval for the difference of RMSE is -0.85 - 1.26, indicating that there is evidence to support
...
# which indicates that the models show successfully difference in performance.  --> THIS IS NOT...
...
plsFit <- pls::plsr(pm ~ ., data = trainingData2, scale = TRUE, center = TRUE, validation = c("CV"))
pcrFit <- pls::pcr(pm ~ ., data = trainingData2, scale = TRUE, center = TRUE, validation = c("CV"))



# ----------
# A contrast of the relationship between each of the first two PCR and PLS components with the fat response.
tmp1 <- data.frame(pm = trainY$pm, comp = plsFit$scores[,1], class = "PLScomp1")
tmp2 <- data.frame(pm = trainY$pm, comp = plsFit$scores[,2], class = "PLScomp2")
tmp3 <- data.frame(pm = trainY$pm, comp = pcrFit$scores[,1], class = "PCRcomp1")
tmp4 <- data.frame(pm = trainY$pm, comp = pcrFit$scores[,2], class = "PCRcomp2")
tmp <- rbind(tmp1, tmp2, tmp3, tmp4)

graphics.off()
trellis.par.set(theme1)
xyplot(pm ~ comp | class, data = tmp)
