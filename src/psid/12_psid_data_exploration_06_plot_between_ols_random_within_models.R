...
# ----------
plotplm(plm(log(income) ~ as.numeric(year), data = ps.p), N = 10, seed = 4, lgth = .05)

plotplm(plm(log(income) ~ as.numeric(year) + sex + as.numeric(age) + educ, data = ps.p), N = 10, seed = 4, lgth = .05)



# -->
# interestingly, negative coefficient in within model !!!



# ----------
mjsp <- data.frame(rbind(jsp, jsp), subject = factor(rep(c("english", "math"), c(3236, 3236))), score =  c(jsp$english, jsp$math))
...
tmp <- mjsp %>% filter(social %in% c(1,2,3,4,5), school %in% c(15,16,17))

ggplot(tmp, aes(x = year, y = score, group = subject)) + geom_line() + facet_wrap(~ id)
