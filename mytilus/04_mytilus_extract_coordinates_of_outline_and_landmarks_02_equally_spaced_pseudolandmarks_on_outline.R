...
# ------------------------------------------------------------------------------
# Extract equally spaced pseudolandmarks on the outline
# ------------------------------------------------------------------------------

k <- 32

Rcx <- (Rc$X[seq(1, length(Rc$X), length = k + 1)])[-1]

Rcy <- (Rc$Y[seq(1, length(Rc$Y), length = k + 1)])[-1]

plot(Rc$X, Rc$Y, type = "l", lwd = 1.5, asp = 1, axes = F, main = "curvilinear")
