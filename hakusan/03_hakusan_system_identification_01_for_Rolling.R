...
# ------------------------------------------------------------------------------
# Estimate ARX model parameter na, nb, nk   Rudder --> Rolling
# ------------------------------------------------------------------------------
...
# ----------
# now we try na = 1, nb = 1, nk = 0

modarx <- arx(ze2, order = c(1,1,0), lambda = 0.1)
...
tmp <- idframe(output = ts(pred_arx), input = ts(ze2$input), Ts = 1)

tmp_t <- idframe(output = ts(pred_arx_t), input = ts(zr2$input), Ts = 1)
...
lsimplot(sys, u = c(ze2$input), t = 1:601)
...
for(i in 1:nrow(parms)){
  ...
  e <- try(
    modoe0 <- oe(ze2, order = c(na, nb, nk)),
    silent = TRUE
  )
  if( class(e) == "try-error") {
  }else{
...
# ----------
# now we try na = 6, nb = 7, nk = 1

modoe <- oe(ze2, order = c(6,7,1))
...
# ----------
# test data
pred_oe <- predict(modoe, ze2)

pred_oe_t <- predict(modoe, zr2)
...
tmp <- idframe(output = ts(pred_oe), input = ts(ze2$input), Ts = 1)

tmp_t <- idframe(output = ts(pred_oe_t), input = ts(zr2$input), Ts = 1)
...
for(i in 1:nrow(parms)){
  ...
  e <- try(
    modarmax0 <- armax(ze2, order = c(na, nb, nc, nk)),
    silent = TRUE
  )
  if( class(e) == "try-error") {
  }else{
...
# ----------
modarmax <- armax(ze1, order = c(5,4,3,1))
...
# ----------
# test data
pred_armax <- predict(modarmax, ze2)

pred_armax_t <- predict(modarmax, zr2)
...
tmp <- idframe(output = ts(pred_armax), input = ts(ze2$input), Ts = 1)

tmp_t <- idframe(output = ts(pred_armax_t), input = ts(zr2$input), Ts = 1)
