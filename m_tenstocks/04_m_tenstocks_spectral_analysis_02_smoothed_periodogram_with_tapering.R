...
# for(i in 2:11){ astsa::mvspec(mtenstocks[,i], kernel = ker, taper = 0.1, log = "no", main = paste0(colnames(mtenstocks)[i])) }

for(i in 1:10){ astsa::mvspec(rtn[,i], kernel = ker, taper = 0.1, log = "no", main = paste0(colnames(rtn)[i])) }
