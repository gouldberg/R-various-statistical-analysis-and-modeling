...
plot(log(fitted(modp)), log((na.omit(CodParasites)$intensity - fitted(modp))^2), xlab=expression(hat(mu)), ylab=expression((y-hat(mu))^2))
# plot(log(rawres^2) ~ log(linpred), xlab = "log(linpred)", ylab = "log(raw resid^2)")

# abline(0,1)
lines(lowess(log(fitted(modp)), log((na.omit(CodParasites)$intensity - fitted(modp))^2)), col = "blue", lty = 2, lwd = 3)
...
with(modp, deviance / df.residual)
...
( phi <- sum(residuals(modp, type = "pearson")^2 / modp$df.residual) )
...
# -->
# Indicates that standard errors of coefficients in this model should be multipled by 1.35, a 35% increase to correct for overdispersion
