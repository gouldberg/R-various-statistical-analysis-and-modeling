...
corrplot(cor(gala), method = "color", cl.length = 20, order = "AOE", addcoef.col = "grey")

corrplot(cor(gala), order = "hclust", hclust.method = "ward.D2", addrect = 3)
...
vc <- varclus(~ ., data = gala, sim="hoeffding")
...
( cor_obj <- cor.mtest(gala, conf.level = 0.95, method = "pearson") )
...
rownames(cor_obj_upper) <- colnames(gala)
colnames(cor_obj_upper) <- colnames(gala)
rownames(cor_obj_lower) <- colnames(gala)
colnames(cor_obj_lower) <- colnames(gala)
