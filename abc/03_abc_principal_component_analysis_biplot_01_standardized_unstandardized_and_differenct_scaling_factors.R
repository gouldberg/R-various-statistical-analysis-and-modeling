...
pca_abc1 <- prcomp(ABC2)

pca_abc2 <- prcomp(ABC2, scale = TRUE)
...
biplot(pca_abc1, pc.biplot = TRUE, cex = c(0.6, 0.8), col = c("gray", "coral1"), arrow.len = 0.05, 
       main = "Biplot (Unstandardized)", xlim = c(-4, 4), asp = 1, cex.axis = 0.8)

biplot(pca_abc2, pc.biplot = TRUE, cex = c(0.6, 0.8), col = c("gray", "coral1"), arrow.len = 0.05, 
...
# -->
# Both left panels are now metric preserving (alpha = 0), meaning that they approximate the Euclidean distances among the each measured points.
...
# in addition to the Mahalanobis distances between measured points.



# ----------
# For the bottom right panel:
round(cor(yaass[,1:5]), 3)
